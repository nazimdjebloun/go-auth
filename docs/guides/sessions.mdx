...
| `TouchDebounce` | `*time.Duration` | Optional | `5m` | Minimum interval between `last_active_at` writes — what makes `last_active_at` in the list responses a few minutes stale rather than exact. Same `nil`-vs-pointer rule as `GraceWindow`; `goauth.Duration(0)` writes on every authenticated request. |
