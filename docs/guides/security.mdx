...
| `CSRFToken` | `*middleware.CSRFTokenConfig` | Optional | `nil` (disabled) | Adds a double-submit cookie token on top of origin checking — this one *can't* be spoofed by just setting a header, since it requires a cookie value only the server issued. |
