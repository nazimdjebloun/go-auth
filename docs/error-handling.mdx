...
Calling the library directly — `auth.Register(...)`, `auth.Services.Auth.Login(...)`, `auth.Services.Admin.BanUser(...)`, and so on — skips the JSON envelope above entirely: you get back a plain Go `error`. Every error the library returns is a `*domain.AuthError` under the hood (the same `Code`/`Message`/HTTP-status triple the JSON envelope serializes), so the two idiomatic `errors` package patterns both work as expected:
...
if err != nil {
    ...
    if errors.As(err, &authErr) {
        // authErr.Code, authErr.Message, authErr.HTTPStatus — the same
        // triple you'd get from the JSON envelope's "error"/"message"
        // fields plus the response status
    }
}
```
