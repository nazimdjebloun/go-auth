...
**Security** — CSRF origin checking with an optional double-submit cookie layer, per-route rate limiting, configurable password policy, tri-state cookie `Secure` and email-link `http://` policy that default sensibly per environment.

**Audit logging** — an async, non-blocking event pipeline with a built-in database sink; add your own sinks (Kafka, NATS, a webhook, anything implementing one interface) to stream the same events elsewhere, with fail-open or fail-closed behavior your choice.
