...
CSRF origin allow-list, password policy, the optional double-submit CSRF cookie, token TTL, and the `http://` policy for email links.
...
goauth.WithSecurity(goauth.SecurityConfig{
    ...
    CSRFToken: &middleware.CSRFTokenConfig{ // optional, default nil (disabled)
...
| `CSRFToken` | `*middleware.CSRFTokenConfig` | Optional | `nil` (disabled) | Adds a double-submit cookie token on top of origin checking. See table below. |
...
**`CSRFToken` (`middleware.CSRFTokenConfig`)** — only relevant if you set it; every field below is optional and defaults when omitted.
...
- `email: Mailer or Email config required when RequireEmailVerification, EnableInvite, RequireEmail2FA, or DefaultTwoFactorEnabled is enabled` — hint: call `WithMailer` or `WithEmail` too.
...
- `email: Mailer or Email config required when RequireEmailVerification, EnableInvite, RequireEmail2FA, or DefaultTwoFactorEnabled is enabled` (repeated from `WithRegistration` — it's the same check).
