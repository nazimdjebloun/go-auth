...
func DefaultRateLimitConfig() *Config {
	return &Config{
		...
		Routes: map[string]Rate{
			...
			// Two-factor
			"POST /auth/2fa/verify":  {Requests: 5, Window: time.Minute},
			"POST /auth/2fa/resend":  {Requests: 3, Window: time.Minute},
			"POST /auth/2fa/enable":  {Requests: 5, Window: time.Minute},
			"POST /auth/2fa/disable": {Requests: 5, Window: time.Minute},
