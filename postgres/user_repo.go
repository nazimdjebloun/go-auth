...
func (r *UserRepository) GetByID(ctx context.Context, id string) (*domain.User, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT id, email, password_hash, name, role, is_verified, is_banned, banned_at, created_at, updated_at
		FROM users WHERE id = $1`, id).Scan(
		&user.ID, &user.Email, &user.PasswordHash, &user.Name, &user.Role,
		&user.IsVerified, &user.IsBanned, &bannedAt, &user.CreatedAt, &user.UpdatedAt)
...
func (r *UserRepository) Update(ctx context.Context, user *domain.User) error {
	_, err := r.db.ExecContext(ctx, `
		UPDATE users SET email=$1, password_hash=$2, name=$3, role=$4,
			is_verified=$5, is_banned=$6, banned_at=$7, updated_at=$8
		WHERE id=$9`,
		user.Email, user.PasswordHash, user.Name, user.Role,
		user.IsVerified, user.IsBanned, user.BannedAt, user.UpdatedAt, user.ID)
	return err
}
...
func (r *UserRepository) List(ctx context.Context, filter port.UserFilter) ([]domain.User, int, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&u.ID, &u.Email, &u.PasswordHash, &u.Name, &u.Role,
			&u.IsVerified, &u.IsBanned, &bannedAt, &u.CreatedAt, &u.UpdatedAt); err != nil {
			return nil, 0, err
		}
		if bannedAt.Valid {
