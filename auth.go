...
func New(config Config) (*Auth, error) {
	...
	return &Auth{
		...
		CheckSession:       http.HandlerFunc(h.CheckAuth).ServeHTTP,
			...
			RevokeUserSessions: csrfMW(adminMW(authMW(http.HandlerFunc(h.RevokeUserSessions)))).ServeHTTP,
			CreateInvite:       csrfMW(adminMW(authMW(http.HandlerFunc(h.CreateInvite)))).ServeHTTP,
			ListInvites:        adminMW(authMW(http.HandlerFunc(h.ListInvites))).ServeHTTP,
			RevokeInvite:       csrfMW(adminMW(authMW(http.HandlerFunc(h.RevokeInvite)))).ServeHTTP,
			ResendInvite:       csrfMW(adminMW(authMW(http.HandlerFunc(h.ResendInvite)))).ServeHTTP,
		},
		Middleware: MiddlewareGroup{
			Authenticate: authMW,
